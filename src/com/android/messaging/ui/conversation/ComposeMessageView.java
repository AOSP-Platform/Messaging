...
public class ComposeMessageView extends LinearLayout
    ...
    private void updateVisualsOnDraftChanged(boolean hasAttachmentsChanged) {
        ...
        if (draftMessageData.getIsMms()) { // MMS case
            if (hasAttachmentsChanged && draftMessageData.hasAttachments()) {
                new SafeAsyncTask<Void, Void, Long>() {
                    @Override
                    protected Long doInBackgroundTimed(Void... params) {
                        final List<MessagePartData> attachments =
                                draftMessageData.getReadOnlyAttachments();
                        long totalSize = 0;
                    ...
                    protected void onPostExecute(Long size) {
                        mMessageBodySize.setText(Formatter.formatFileSize(getContext(), size));
